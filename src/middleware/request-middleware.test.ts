...
describe('Error Handling Middleware', () => {
  beforeEach(() => {
    next = jest.fn();
    request = new Request('/employees?sort=desc', {
      headers: {
        Accept: 'text/html'
      }
    });
    badRequest = new Request('/employees?sort=desc', {
      headers: {
        Accept: 'text/html'
      }
    });
  ...
  test('Handover request to express error handler on error', async () => {
    ...
    const wrappedRoute = requestMiddleware(sampleRoute);
    await wrappedRoute(request, null as any, next);
  ...
  test('Handover request to express error handler on error', async () => {
    ...
    await wrappedRoute(badRequest, null as any, next);
